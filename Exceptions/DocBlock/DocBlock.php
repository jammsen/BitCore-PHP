...
use DocBlock\Tag;
use DocBlock\Context;
use DocBlock\Location;
...
class DocBlock implements \Reflector
{
    ...
    public static function export(&$var) // add Options??
    {
        try{
            $ret = '<p>'.$var->getShortDescription().'<br><br>'.$var->getLongDescription().'</p>'.PHP_EOL;
            foreach($var->getTags()as $tag)
            {
                $ret .= $tag."<br>";
            }
        }catch(Exception $e)
